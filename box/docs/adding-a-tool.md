...
6. Sync + linkage: add the launcher and generated `verify-<id>.sh` to `Makefile` `SHELL_FILES` (CI calls `make verify-static`, so no workflow edit is needed); extend `tests/bats/config-linkage.bats` (registry row, `3 + 5 × tool_count` partials, `--dry-run` shape) and `tests/bats/makefile.bats` (`make -n build-<stem>` delegation).
7. Generate + gate: `bash gen-verify.sh && bash gen-pins.sh` (never hand-edit generated outputs), then `make verify-static && make pins`.

Do not touch: `lib/preflight.sh`, `lib/docker.sh`, `lib/run.sh`, `lib/pins.sh`, `lib/build.sh`, `check-pins.sh`, or `Makefile` rules (only the `SHELL_FILES` list). Exceptions with acknowledged tool-specific blocks that need edits for any similar future tool: the muse-only persist block + seed in `setup.sh` (persist dir, seed call) and its opencode-owned `NODE_VERSION` status line, the muse safety-key cross-check in `gen-pins.sh` (and its opencode-owned Node rows), plus `update-pins.sh` (`resolve_<id>` + `new_pin` entries + `BOX_UPDATE_*` seeds). New credential key? Add one token to `BOX_CRED_KEYS` in `lib/config.sh` (shared data, not branching).
